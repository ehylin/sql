...
CREATE TABLE coches.GrupoEmpresarial (
...
CREATE TABLE coches.Marca (
    ...
    nombre VARCHAR(100) NOT NULL,
    idGrupo INTEGER REFERENCES coches.GrupoEmpresarial(idGrupo)
);

CREATE TABLE coches.Modelo (
    idModelo SERIAL PRIMARY KEY,
    nombre VARCHAR(100) NOT NULL,
    idMarca INTEGER REFERENCES coches.Marca(idMarca)
);
...
CREATE TABLE coches.Coche (
    ...
    kmsTotales INT,
    fechaCompra DATE,
    idModelo INTEGER REFERENCES coches.Modelo(idModelo)
);
...
CREATE TABLE coches.Aseguradora (
    ...
    nombre VARCHAR(100) NOT NULL,
    nPoliza INT,
    fechaAltaSeguro DATE,
    matricula VARCHAR(50) REFERENCES coches.Coche(matricula)
);
...
FROM 
    coches.Coche c
JOIN 
    coches.Modelo m ON c.idModelo = m.idModelo
JOIN 
    coches.Marca ma ON m.idMarca = ma.idMarca
JOIN 
    coches.GrupoEmpresarial g ON ma.idGrupo = g.idGrupo
JOIN 
    coches.Aseguradora a ON c.matricula = a.matricula
